classdef Conv < dagnn.Filter
    ...
    methods
        function outputs = forward(obj, inputs, params)
            ...
            inputSizes = cellfun(@(x) [size(x) 1 1 1 1], inputs, 'uni', 0);
            outputSizes = obj.getOutputSizes(inputSizes);
            if all(outputSizes{1}>0)
                outputs{1} = vl_nnconv(...
                ...
                outputs{1} = gpuArray.zeros(max(outputSizes{1},1),'single');
            end
        end
...
%                 sc = sqrt(2 / (prod(obj.size(1:3)) + obj.size(4))) ;
            else
                sc = obj.initScale;
            end
